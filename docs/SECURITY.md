...
- Profile-governed tools require a separate per-action operator decision. The daemon pauses before side effects, hashes the run ID, canonical registry tool name, and canonical arguments, and persists only that hash plus bounded redacted previews. Approve/deny requests must echo the displayed hash. Execution revalidates current permission, tool authority, payload hash, status, and expiry, then atomically consumes the approval once before invoking the handler.
...
- Durable run-artifact capture accepts only supported kind/MIME pairs, bounded JSON metadata, bounded content, and credential-free HTTPS external URLs. Workspace sources must be relative regular files inside the project root; traversal, symbolic links, hard links, protected runtime/Git/credential paths, and oversized sources are rejected. Captured bytes are copied exclusively into owner-only `.nuaai/artifacts/` storage and verified against their recorded size and SHA-256 before download.
