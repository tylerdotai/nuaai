...
- HTTP API routes require a generated HMAC bearer token. Loading the static browser shell issues no credential. `nuaai pair [base-url]` creates a five-minute, browser-pairing-only token in the URL fragment. The client removes that fragment from browser history before sending the exchange request, then receives a separate 30-day HTTP-only, same-site cookie scoped to the actual app mount. Pairing tokens cannot authenticate HTTP API or Server-Sent Events routes, and runtime bearer tokens cannot be exchanged for browser cookies.
- Server-Sent Events connections authenticate via token at connect time and stream runtime events; reconnect replays buffered events through `Last-Event-ID` to avoid missed state. A 15-second heartbeat prevents idle proxies from closing the stream.
...
- Authenticated Server-Sent Events stream carries approval-lifecycle events alongside runtime events, filtered by session scope and replayable through `Last-Event-ID` on reconnect. The PWA generation-fences concurrent refreshes so stale responses cannot restore a decided or expired action.
