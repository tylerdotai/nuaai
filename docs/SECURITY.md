...
NUAAI is designed for one local operator on a loopback-bound host.
...
- The token is stored in `.nuaai/runtime.json`, which is Git-ignored and never printed at daemon startup.
...
- `.nuaai/`, `.env*`, runtime identity files, master keys, and generated build artifacts are excluded from Git.
