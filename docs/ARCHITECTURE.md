...
3. **Action:** a structured provider tool call is validated against the advertised catalog and executed once through the daemon-owned registry. A provider-owned loop must pass the exact call ID and qualified advertised name into the runtime-supplied dynamic-tool callback; lifecycle events become evidence only when their call ID, name, argument hash, result hash, and status match that callback execution.
...
6. **Approval:** each request binds the run ID, canonical registry tool name, and canonical arguments into a SHA-256 hash. SQLite stores the hash plus a versioned tool-specific allowlisted preview—not raw arguments—and atomically consumes an approved request once immediately before execution. Workspace content, typed values, prompts, command arguments, and similar opaque text are represented only by labeled sizes and SHA-256 fingerprints; safe paths, desktop actions, coordinates, element references, and authenticated client/session context remain intelligible. Pending requests survive daemon restart; a resumed provider must propose the exact same payload hash before execution.
...
Before each provider turn, the runtime applies a provider-neutral token budget using a deterministic UTF-8 estimator. It reserves system-prompt, tool-schema, current-input, and response headroom; keeps project/session constraints and pinned memory; selects recent complete turns; and treats each structured assistant tool-call artifact plus all matching tool-result messages as one atomic group. Incomplete groups and orphan tool rows never cross the provider boundary. The assembled system prompt travels once in the dedicated provider request field; adapters canonicalize any legacy system rows into that field before producing their wire body, and token metrics account for the canonical single copy.
...
Token pressure creates a durable deterministic extractive checkpoint without deleting transcript rows. Each checkpoint records source start/end message IDs, source message count, canonical source SHA-256 and provenance version, estimated original/summary tokens, the exact summary budget, summarizer and policy versions, checkpoint version, and update time. Reuse requires the current source hash, budget, policy, summarizer, and recomputed summary estimate to remain compatible; budget or policy changes regenerate the checkpoint before it enters provider context. Rolling checkpoints retain previously selected records when possible, never cut a UTF-8 string or source record, and produce identical provider context after restart. `context.compacted` and `context.selected` events expose counts and token estimates only, never message content.
...
Ollama uses the local HTTP API for streaming chat and embeddings. Codex turns use the fixed HTTPS ChatGPT Codex Responses endpoint with OAuth state read on demand from the installed Codex CLI. NUAAI never copies OAuth state into project configuration or SQLite. When an access token nears expiry, NUAAI invokes a short-lived, sanitized `codex app-server` account RPC so Codex refreshes its own store; model turns do not use App Server. Responses function calls normalize into the same daemon-owned, permission-filtered tool loop as local providers. Built-in providers currently use the NUAAI-owned loop (`ownsToolLoop !== true`); future provider-owned adapters must implement exact dynamic-tool call metadata and attested lifecycle matching. Visible model IDs come from Codex's local model cache, and independent provider selections survive restarts. The deterministic provider exists only under `NUAAI_TEST_MODE=1`.
